...
def downloader(url,path):
    ...
    r = requests.get(url,headers=headers,stream=True)  #stream必须要有
    chunk_size = 1024                               #每次下次的数据
    ...
    if r.status_code == 200:
        ...
        if not os.path.exists(path):
            ...
            with open(path,'wb') as f:
                for data in tqdm(r.iter_content(chunk_size=chunk_size)):   #Response.iter_content 可以边下载边存硬盘
                    f.write(data)
            stop = time.time()

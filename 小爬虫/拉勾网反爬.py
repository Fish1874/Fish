...
db = conn.lagou  #没有这个数据库， 则自动创建
mongo_lagou = db.job #使用job合集，没有自动创建
...
headers = {
        'User-Agent':random.choice(user_agent), # 随机选取头部代理
        'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
        'Referer': 'https://www.lagou.com/jobs/list_python?city=%E5%B9%BF%E5%B7%9E&cl=false&fromSearch=true&labelWords=&suginput=&labelWords=hot'
...
proxy = {'http:': 'http://{}:{}'.format(proxy['ip'], proxy['port'])}
# 获取详情页   职位描述信息！
def getInfo(url):
    # url在下面获取了用户ID就构成完成的传入这里
...
for page in range(1,2):
    ...
    s = requests.session()
    s.get(url1, headers=headers, timeout=2)
    cookies = s.cookies  # 获取此次cookies
    ...
    for position in positions:
        ...
        time.sleep(3)
        # 再加上随机延迟   numpy.random.rand()随机返回浮点数
